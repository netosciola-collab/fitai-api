import { WorkoutDay, WorkoutExercise } from "@prisma/client";
...
interface CheckInResult {
  score: number;
  decision: "TREINO_NORMAL" | "TREINO_MODERADO" | "TREINO_LEVE" | "RECUPERACAO_ATIVA";
  aiRationale: string;
...
export async function processCheckIn(
  ...
  workoutDay: WorkoutDay & { exercises: (WorkoutExercise & { exercise: any })[] }
): Promise<CheckInResult> {
  // Calcular score ponderado
  const score =
    (checkInData.sleepQuality * 1.5 +
  ...
  let decision: CheckInResult["decision"];
  let aiRationale: string;
  let adjustedExercises: AdjustedExercise[] = [];
  ...
  if (score >= 4.0) {
    ...
    adjustedExercises = workoutDay.exercises
      .map((ex, idx) => {
        const isIsolated = !["supino", "agachamento", "rosca", "puxada"].some((c) =>
          ex.exercise.name.toLowerCase().includes(c)
        );
      ...
      .filter(Boolean) as AdjustedExercise[];
  } else if (score >= 2.0) {
  ...
  if (checkInData.availableTime < 30) {
    ...
    adjustedExercises = adjustedExercises.filter((ex) => {
      const isIsolated = !["supino", "agachamento", "rosca", "puxada"].some((c) =>
        ex.name.toLowerCase().includes(c)
      );
      return !isIsolated;
    });
